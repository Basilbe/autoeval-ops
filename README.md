...
hallucination (lexical grounding), cost, latency
        ...
        ├──►  Persisted to Postgres (evaluations, eval_results, traces)
...
3. **Add prompt files** to your repo:
   ...
   - `eval/summarize.test_cases.json` — a matching file with test
...
a Clerk application, and a `GOOGLE_API_KEY` — see

[`docs/TECH_STACK.md`](docs/TECH_STACK.md) for what each is for.
